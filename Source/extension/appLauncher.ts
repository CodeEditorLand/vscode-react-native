...
export class AppLauncher {
	...
	public getOrUpdateNodeModulesRoot(forceUpdate = false): string {
		if (!this.nodeModulesRoot || forceUpdate) {
			const nodeModulesRootPath: string | null =
				getNodeModulesInFolderHierarchy(this.packager.getProjectPath());
	...
	public async launch(launchArgs: any): Promise<any> {
		...
		if (!isNullOrUndefined(launchArgs.launchActivity)) {
			mobilePlatformOptions.debugLaunchActivity =
				launchArgs.launchActivity;
		}
	...
	private async resolveAndSaveMobileTarget(
		...
		if (
			launchArgs.target &&
			!(await mobilePlatform.getTargetFromRunArgs())
		) {
			const isAnyTarget =
