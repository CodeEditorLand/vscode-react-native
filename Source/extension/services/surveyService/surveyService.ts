...
export class SurveyService implements vscode.Disposable {
	...
	private extensionFirstTimeInstalled = false;
	...
	public async promptSurvey(): Promise<void> {
		...
		if (this.surveyConfig.daysLeftBeforeSurvey === 0) {
			if (this.isCandidate()) {
				...
				this.surveyConfig.daysLeftBeforeSurvey =
					this.surveyConfig.shortPeriodToRemind;
			}
		} else if (
	...
	private async initializeSurveyConfig(): Promise<void> {
		...
		const surveyConfigLocal: SurveyConfig =
			ExtensionConfigManager.config.has(this.SURVEY_CONFIG_NAME)
				...
				: {
						...
						enabled: false,
						daysLeftBeforeSurvey: this.extensionFirstTimeInstalled
							? 30
							: 3,
						surveyName: "none",
						surveyUrl: "",
	...
	private sendPromptSurveyTelemetry(surveyName: string): void {
		const promptUserSurveyEvent = TelemetryHelper.createTelemetryEvent(
			"promptUserSurvey",
			{
				surveyName,
			},
