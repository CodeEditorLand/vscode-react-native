...
const processExecutionsRecordingsPath = path.join(
	__dirname,
	"processExecutionsRecordings"
);

// This class simulates calling the React-Native CLI v0.65.
export class ReactNative065 {
	public static DEFAULT_PROJECT_FILE = path.join(
		sampleRNProjectPath,
		"package.json"
	);

	private static ANDROID_APK_RELATIVE_PATH =
	...
	public loadRecordingFromString(recordingContent: string): Promise<void> {
		return Promise.resolve(
			this.loadRecording(JSON.parse(recordingContent))
		);
	}
	...
	public createProject(
		...
		return Promise.resolve()
			...
			.then((defaultContents) => {
				const reactNativeConfiguration = JSON.parse(
					defaultContents.toString()
				);
				reactNativeConfiguration.name = projectName;
				const reactNativeConfigurationFormatted = JSON.stringify(
			...
			.then(() => {
				return this.fileSystem.mkDir(
					this.getAndroidProjectPath(projectRoot)
				);
			});
	}
	...
	private readAndroidPackageName(): Promise<void> {
		return new Package(this.projectRoot, { fileSystem: this.fileSystem })
			.name()
			.then((name) => {
	...
	private createAPK(): Promise<void> {
		return this.isAndroidProjectPresent()
			...
			.then(() => {
				...
				return new APKSerializer(this.fileSystem).writeApk(
					this.androidAPKPath,
					{
	...
	private installAppInAllDevices(): Promise<void> {
		...
		return PromiseUtil.reduce(devices, (device) =>
			this.installAppInDevice(device.id)
		);
	}
        ...
        **/
		...
		if (matches) {
			if (
				...
				return this.adbHelper.launchApp(
					this.projectRoot,
