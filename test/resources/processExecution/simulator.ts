...
import {
	ISpawnResult,
	ChildProcess,
} from "../../../src/common/node/childProcess";
...
export class Simulator {
	...
	private simulateOutputSideEffects(
		...
		this.wholeOutputBasedDefinitions =
			this.wholeOutputBasedDefinitions.filter((definition) => {
				const match = this.allStdout.match(
					definition.wholeOutputPattern
				);
				if (match && match.index !== undefined) {
					applicableSideEffectDefinitions.push({
						index: match.index,
		...
		return PromiseUtil.reduce(
			applicableSideEffectDefinitions,
	...
	private simulateSingleEvent(event: IEventArguments): Promise<void> {
		...
		return new Promise((resolve) => {
			...
			const key = Object.keys(event).find(
				(eventKey) => eventKey !== "after"
			); // At the moment we are only using a single key/parameter per event
			switch (key) {
				case "stdout": {
					const data = (<IStdOutEvent>event).stdout.data;
					const previousOutputLength = this.allStdout.length;
					this.allStdout += data;
					this.simulateOutputSideEffects(
						data,
				...
				default:
					throw new Error(
						`Unknown event to simulate: ${key} from:\n\t${event}`
					);
			}
			return resolve();
		});
