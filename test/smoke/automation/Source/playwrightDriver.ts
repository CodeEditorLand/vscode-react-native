...
function buildDriver(
	...
	const driver: IDriver = {
		...
		exitApplication: async () => {
			try {
				await context.tracing.stop({
					path: join(
						logsPath,
						`playwright-trace-${traceCounter++}.zip`
					),
				});
			} catch (error) {
		...
		click: async (windowId, selector, xoffset, yoffset) => {
			const { x, y } = await driver.getElementXY(
				windowId,
				selector,
				xoffset,
				yoffset
		...
		setValue: async (windowId, selector, text) =>
			page
				.evaluate(`window.driver.setValue('${selector}', '${text}')`)
				.then(undefined),
		...
		getElements: (windowId, selector, recursive) =>
			page.evaluate(
				`window.driver.getElements('${selector}', ${recursive})`
			),
		getElementXY: (windowId, selector, xoffset?, yoffset?) =>
			page.evaluate(
				`window.driver.getElementXY('${selector}', ${xoffset}, ${yoffset})`
			),
		typeInEditor: (windowId, selector, text) =>
			page.evaluate(
				`window.driver.typeInEditor('${selector}', '${text}')`
			),
		...
		writeInTerminal: (windowId, selector, text) =>
			page.evaluate(
				`window.driver.writeInTerminal('${selector}', '${text}')`
			),
		getLocaleInfo: (windowId) =>
			page.evaluate(`window.driver.getLocaleInfo()`),
		getLocalizedStrings: (windowId) =>
			page.evaluate(`window.driver.getLocalizedStrings()`),
	};
	return driver;
}
	...
	if (server) {
		try {
			await new Promise<void>((c, e) =>
				kill(server!.pid, (err) => (err ? e(err) : c()))
			);
		} catch {
			// noop
		}
...
export function connect(
	...
	return new Promise(async (c) => {
		...
		page.on("crash", (page) =>
			console.error("Playwright ERROR: page crash")
		);
