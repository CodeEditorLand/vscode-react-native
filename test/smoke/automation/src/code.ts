...
export async function spawn(options: SpawnOptions): Promise<Code> {
	...
	if (options.web) {
		...
		connectDriver = connectPlaywrightDriver.bind(
	...
	if (options.remote) {
		...
		args[0] = `--${
			options.workspacePath.endsWith(".code-workspace")
				? "file"
				: "folder"
		}-uri=vscode-remote://test+test/${
			URI.file(options.workspacePath).path
		}`;
...
export class Code {
	...
	public async waitForWindowIds(
		fn: (windowIds: number[]) => boolean
	): Promise<void> {
