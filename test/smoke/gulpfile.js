...
gulp.task("download-vscode-repo", (done) => {
	console.log(
		`*** Downloading VS Code ${CODE_REPO_VERSION} repo into directory: ${CODE_ROOT}`
	);
...
gulp.task("remove-vscode-smoke-tests", (done) => {
	console.log(
		`*** Removing VS Code repo smoke tests directory: ${CODE_SMOKE_TESTS_FOLDER}`
	);
	rimraf.sync(CODE_SMOKE_TESTS_FOLDER);
	console.log(
		`*** Removing VS Code repo smoke tests directory: ${CODE_AUTOMATION_FOLDER}`
	);
	...
	gulp.series(
		...
		function copyPackage(done) {
			...
			ncp(SMOKE_TESTS_PACKAGE_FOLDER, CODE_SMOKE_TESTS_FOLDER, (err) => {
				...
				ncp(
					SMOKE_TESTS_AUTOMATION_FOLDER,
					CODE_AUTOMATION_FOLDER,
					(err) => {
						if (err) {
							console.error(
