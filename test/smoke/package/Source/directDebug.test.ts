...
export function startDirectDebugTests(
	...
	describe("Direct debugging", () => {
		...
		async function hermesApplicationTest(
			testname: string,
			platform: Platform
		) {
			try {
				if (
					...
					return assert.fail(
						`Passed unsupported platform: ${platform}`
					);
				}
				...
				SmokeTestLogger.info(
					`${testname}: Attaching to app via Appium`
				);
				...
				let isHermesWorking = await AppiumHelper.isHermesWorking(
					client,
					platform
				);
				...
				await automationHelper.waitForStackFrameWithRetry(
					...
					async () =>
						await AppiumHelper.clickTestButton(
							client ||
								(await AppiumHelper.webdriverAttach(opts)),
