...
export function startOtherTests(
	project: TestProject,
	...
	describe("React Native", () => {
		...
		if (!testParameters || testParameters.RunAndroidTests) {
			it("Select and save Android emulator test", async function () {
				...
				const launchConfigurationManager =
					new LaunchConfigurationManager(project.workspaceDirectory);
				app = await initApp(
				...
				launchConfigurationManager.updateLaunchScenario(
				...
				try {
					...
					await app.workbench.quickinput.submit(
						androidEmulatorManager.getEmulatorName()
					);
					SmokeTestLogger.info(
				...
				SmokeTestLogger.info(
					"Select and save Android emulator test: Dispose all"
				);
				await disposeAll();
				app = await initApp(
					project.workspaceDirectory,
		...
		if (
			...
			it("Select and save iOS simulator test", async function () {
				...
				let isStarted =
					await iosSimulatorManager.waitUntilIosSimulatorStarting();
				assert.strictEqual(
					isStarted,
					true,
