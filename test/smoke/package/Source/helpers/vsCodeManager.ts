...
export class VsCodeManager {
	...
	private static VS_CODE_CLIENT_NOT_INSTALLED_ERROR =
		"VS Code client was not installed";
	private static CURRENT_SESSION_LOGS_DIR_ERROR =
		"Сurrent session logs directory is not defined";
	...
	constructor(
		...
		this.clientVersion = process.env.CODE_VERSION
			? process.env.CODE_VERSION
			: "stable";

		this.artifactDirectory = path.join(
			projectRoot,
			SmokeTestsConstants.artifactsDir
		);
		...
		this.vsCodeClientCmdDirectory =
			vscodeTest.resolveCliPathFromVSCodeExecutablePath(
	...
	public async downloadVSCodeExecutable(): Promise<any> {
		SmokeTestLogger.projectInstallLog(
			"*** Downloading VS Code executable..."
		);
		...
		this.vsCodeClientAppFileDirectory =
			await vscodeTest.downloadAndUnzipVSCode(
				this.clientVersion,
	...
	public cleanUp(): void {
		...
		if (fs.existsSync(this.artifactDirectory)) {
			SmokeTestLogger.info(
				`*** Deleting test logs directory: ${this.artifactDirectory}`
			);
	...
	public installExtensionFromVSIX(deleteVSIX: boolean = false): void {
		if (this.clientIsInstalled) {
			...
			let extensionFile = utilities.findFile(
				this.vsixDirectory,
				/.*\.(vsix)/
			);
			if (!extensionFile) {
			...
			utilities.spawnSync(this.vsCodeClientCmdDirectory, args, {
				stdio: "inherit",
			});

			if (deleteVSIX) {
				SmokeTestLogger.info(
					`*** Deleting ${extensionFile} after installation`
				);
	...
	public killWinCodeProcesses(): void {
		...
		try {
			...
			this.taskKillCommands.forEach((cmd) => {
				...
				const result = utilities.execSync(
					cmd,
					undefined,
	...
	public async runVSCode(
		...
		if (this.clientIsInstalled) {
			...
			SmokeTestLogger.info(
				`Options for run ${dirName}: ${JSON.stringify(
					options,
					null,
					2
				)}`
	...
	private createOptions(
		...
		if (this.clientIsInstalled) {
			...
			SmokeTestLogger.info(
				`*** Executing ${this.vsCodeClientAppFileDirectory}`
			);
	...
	public findStringInLogs(string: string, logFile: string): boolean {
		if (this.currentSessionLogsDir) {
			SmokeTestLogger.info(
				`*** Searching for \"${string}\" string in output file`
			);
