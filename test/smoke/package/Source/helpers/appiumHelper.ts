...
export class AppiumHelper {
	...
	public static XPATH: XPathSelectors = {
		RN_RELOAD_BUTTON: {
			...
			[Platform.iOSExpo]:
				"//XCUIElementTypeOther[@name='Reload JS Bundle']",
		},
		RN_ENABLE_REMOTE_DEBUGGING_BUTTON: {
			[Platform.Android]:
				"//*[@text='Debug JS Remotely' or @text='Debug']",
			[Platform.AndroidExpo]: "//*[@text='Debug Remote JS']",
			[Platform.iOS]:
				"//XCUIElementTypeButton[@name='Debug JS Remotely' or @name='Debug']",
			[Platform.iOSExpo]:
				"//XCUIElementTypeOther[@name='󰢹 Debug Remote JS']",
		},
		RN_STOP_REMOTE_DEBUGGING_BUTTON: {
			[Platform.Android]:
				"//*[@text='Stop Remote JS Debugging' or @text='Stop Debugging']",
			...
			[Platform.iOSExpo]:
				"//XCUIElementTypeOther[@name=' Stop Remote Debugging']",
		},
		RN_DEV_MENU_CANCEL: {
	...
	public static runAppium(appiumLogPath: string): void {
		SmokeTestLogger.info(
			`*** Executing Appium with logging to ${appiumLogPath}`
		);
		let appiumCommand =
		...
		appiumProcess.on("error", (error) => {
			SmokeTestLogger.error(
				`Error occurred in Appium process: ${error.toString()}`
			);
		});
	}
		...
		if (appiumProcess) {
			...
			if (process.platform == "win32") {
				retrieveProcessesData = () =>
					cp.execSync("tasklist").toString();
			} else {
				retrieveProcessesData = () => {
					try {
						return appiumProcess
							? cp
									.execSync(`ps -p ${appiumProcess.pid}`)
									.toString()
							: "";
					} catch (err) {
			...
			const condition = () => {
				if (
					appiumProcess &&
					retrieveProcessesData().includes(String(appiumProcess.pid))
				) {
					SmokeTestLogger.info(
		...
		return {
			capabilities: {
				...
				platformVersion:
					...
					SmokeTestsConstants.defaultTargetIosPlatformVersion,
				deviceName: deviceName,
				app: appPath,
	...
	public static webdriverAttach(
		attachArgs: wdio.RemoteOptions
	...
	public static async openExpoApplication(
		...
		if (platform === Platform.Android) {
			if (process.platform === "darwin") {
				...
				return this.openExpoAppViaExpoXDLAndroidFunction(
					client,
					projectFolder
				);
			} else {
			...
			return this.openExpoAppViaExpoXDLSimulatorFunction(
				client,
				projectFolder,
				firstLaunch
			);
	 ...
	 */
	public static async callRNDevMenu(
		client: AppiumClient,
		platform: Platform
	): Promise<void> {
		switch (platform) {
			...
			case Platform.iOSExpo:
				...
				SmokeTestLogger.info(
					"*** Opening DevMenu by sending shake gesture..."
				);
				await client.shake();
				await sleep(2 * 1000);
				break;
	...
	public static async reloadRNApp(
		client: AppiumClient,
		platform: Platform
	): Promise<void> {
		SmokeTestLogger.info(
			"*** Reloading React Native application with DevMenu..."
		);
		const reloadButton = await client.$(
			this.XPATH.RN_RELOAD_BUTTON[platform]
		);
	...
	public static async enableRemoteDebugJS(
		...
		SmokeTestLogger.info(
			"*** Enabling Remote JS Debugging for application with DevMenu..."
		);
		...
		await client.waitUntil(async () => {
			if (await enableRemoteDebugButton.isExisting()) {
				...
				if (await enableRemoteDebugButton.isExisting()) {
					await enableRemoteDebugButton.click();
					SmokeTestLogger.info(
						"*** Debug JS Remotely button clicked second time..."
					);
				}
				return true;
	...
	public static async disableExpoErrorRedBox(
		client: AppiumClient
	): Promise<void> {
		const dismissButton = await client.$(
			"//XCUIElementTypeButton[@name='redbox-dismiss']"
		);
		if (await dismissButton.isExisting()) {
			SmokeTestLogger.info(
				"*** React Native Red Box found, disabling..."
			);
	...
	public static async disableDevMenuInformationalMsg(
		...
		if (await gotItButton.isExisting()) {
			SmokeTestLogger.info(
				"*** Expo DevMenu informational message found, disabling..."
			);
	...
	public static async clickTestButton(
		...
		SmokeTestLogger.info(
			`*** Pressing button with text "${testButtonName}"...`
		);
		let testButton: any;
		switch (platform) {
			case Platform.Android:
				testButton = await client.$(
					`//*[@text='${testButtonName.toUpperCase()}']`
				);
				break;
			case Platform.iOS:
				testButton = await client.$(
					`//XCUIElementTypeButton[@name="${testButtonName}"]`
				);
				break;
		}
	...
	public static async isHermesWorking(
		...
		switch (platform) {
			case Platform.Android:
				...
				break;
			case Platform.iOS:
				hermesMark = await client.$(
					'//XCUIElementTypeStaticText[@name="Engine: Hermes"]'
				);
				break;
		}
	...
	private static async openExpoAppViaClipboardAndroid(
		client: AppiumClient,
		expoURL: string
	) {
		...
		const exploreElement = await client.$(
			"//android.widget.TextView[@text='Projects']"
		);
		await exploreElement.waitForExist({
			timeout: SmokeTestsConstants.waitForElementTimeout,
		});
		...
		const expoOpenFromClipboard = await client.$(
		...
		SmokeTestLogger.info(
			`*** ${expoOpenFromClipboard.selector} clicked...`
		);
	}
	...
	private static async openExpoAppViaExpoXDLSimulatorFunction(
		...
		await projectsElement.waitForExist({
			timeout: SmokeTestsConstants.waitForElementTimeout,
		});
