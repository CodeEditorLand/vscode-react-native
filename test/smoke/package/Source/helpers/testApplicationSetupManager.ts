...
export class TestApplicationSetupManager {
	...
	constructor(resourcesDirectory: string, cacheDirectory: string) {
		...
		this.windowsHermesTestProject = new TestProject(
			path.join(
				this.testAppsDirectory,
				SmokeTestsConstants.RNWHermesAppName
			),
			path.join(
				resourcesDirectory,
				SmokeTestsConstants.sampleRNWHermesAppName
			)
		);
	...
	public async prepareTestApplications(
		...
		const packagesForPureReactNativeProjects = new Map<string, string>(
			Object.entries({
				"react-native": pureRnVersion,
				expo: pureExpoSdkVersion,
			})
		);
	...
	private static async getLatestSupportedRNVersionForExpo(
		...
		const printSpecifiedMajorVersion = expoSdkMajorVersion
			? `sdk-${expoSdkMajorVersion}`
			: "";
		...
		const content = await (
			await fetch("https://exp.host/--/api/v2/versions")
		).json();
		...
		if (content.sdkVersions) {
			...
			if (
				...
				return content.sdkVersions[usesSdkVersion]
	...
	private prepareWithCacheMiddleware(
		...
		if (
			...
			if (
				project.workspaceDirectory.includes(
	...
	private prepareReactNativeProjectForMacOSHermesApplication(
		...
		const hermesEngineDarwinInstallCommand =
	...
	private prepareReactNativeProjectForHermesTesting(project: TestProject) {
		...
		fs.copyFileSync(
			project.settingsJsonSamplePath,
			project.settingsJsonPath
		);
	}

	private generateReactNativeInitCommand(
		appName: string,
		version?: string
	): string {
		let command = "";
		if (appName === SmokeTestsConstants.RNWAppName) {
			...
			command = `react-native init ${appName}${
	...
	private prepareReactNativeApplication(
		...
		const command = this.generateReactNativeInitCommand(
			project.appName,
			version
		);
		SmokeTestLogger.projectInstallLog(
		...
		if (!fs.existsSync(project.vsCodeConfigPath)) {
			SmokeTestLogger.projectInstallLog(
				`*** Creating  ${project.vsCodeConfigPath}...`
			);
	...
	private prepareExpoApplication(
		...
		const useSpecificSdk = expoSdkMajorVersion
			? `@sdk-${expoSdkMajorVersion}`
			: "";
	...
	private prepareRNWApplication(
		...
		this.prepareReactNativeApplication(project, rnVersion);
		this.prepareReactNativeProjectForWindowsApplication(
			project,
			false,
			rnwPackageVersion
		);
	...
	private prepareTestExpressServer(
		...
		fs.writeFileSync(
			appPackageFilePath,
			fs.readFileSync(resPackageFilePath)
		);
	...
	private copyGradleFilesFromSample(project: TestProject) {
		...
		fs.writeFileSync(
			appGradleBuildFilePath,
			fs.readFileSync(resGradleBuildFilePath)
		);
	}

	private copyPodfileFromSample(project: TestProject, platform: string) {
		const appPodfilePath =
			project.getPodfileByPlatformForWorkspace(platform);
...
module.exports.watchFolders = ['.vscode'];`;
	...
	private removeProjectFolder(project: TestProject) {
		if (fs.existsSync(project.workspaceDirectory)) {
			SmokeTestLogger.info(
				`*** Deleting project directory: ${project.workspaceDirectory}`
			);
	...
	private patchExpoSettingsFile(project: TestProject) {
		...
		if (fs.existsSync(settingsJsonPath)) {
			SmokeTestLogger.projectPatchingLog(
				`*** Patching ${settingsJsonPath}...`
			);
			let content = JSON.parse(
				fs.readFileSync(settingsJsonPath).toString()
			);
			if (content.https === false) {
				SmokeTestLogger.projectPatchingLog(
					`*** Deleting https: ${content.https} line...`
				);
	...
	private useCachedApps(
		...
		try {
			...
			const packageJsonData = JSON.parse(
				String(fs.readFileSync(project.packageJsonPath))
			);
			packagesVersions.forEach((version: string, packageName: string) => {
				if (
					(!packageJsonData.dependencies[packageName] ||
						!packageJsonData.dependencies[packageName].includes(
							version
						)) &&
					(!packageJsonData.devDependencies[packageName] ||
						!packageJsonData.devDependencies[packageName].includes(
							version
						))
				) {
					useCachedApp = false;
	...
	private installPackagesForProject(
		...
		let command = "";
		if (isYarn) {
			command = `${utilities.yarnCommand} add ${packages.join(" ")} ${
				isDev ? "--dev" : ""
			}`;
		} else {
	...
	private execGradlewCleanCommand(project: TestProject): void {
		const commandClean =
			path.join(project.getPlatformFolder("android"), "gradlew") +
			" clean";
	...
	private execPodInstallCommand(
		project: TestProject,
		platform: string
	): void {
		const commandInstall =
			"LANG=en_US.UTF-8 pod install --repo-update --verbose";
