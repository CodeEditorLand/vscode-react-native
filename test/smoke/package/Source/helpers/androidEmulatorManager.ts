...
export default class AndroidEmulatorManager {
	...
	public async getExpoAndroidClientForSDK(
		expoSdkMajorVersion: string
	): Promise<ExpoClientData> {
		const sdkVersion = (await XDL.Versions.sdkVersionsAsync())[
			`${expoSdkMajorVersion}.0.0`
		];
		return {
	...
	public uninstallTestAppFromEmulator(appPackage: string): void {
		SmokeTestLogger.info(
			`*** Uninstalling test app ${appPackage}' from Emulator`
		);
		try {
			...
			SmokeTestLogger.error(
				`Error occured while uninstalling test app:\n ${e.toString()}`
			);
		}
	}
	...
	public async runAndroidEmulator(): Promise<boolean> {
		...
		SmokeTestLogger.info(
			`*** Executing Android emulator with 'emulator ${emulatorOpts.join(
				" "
			)}' command...`
		);
	...
	public async terminateAndroidEmulator(): Promise<boolean> {
		...
		SmokeTestLogger.info(
			`*** Checking for running emulator with id ${this.emulatorId}...`
		);
		if (devices.find((it) => it.id === this.emulatorId)) {
			cp.execSync(`adb -s ${this.emulatorId} emu kill`, {
				stdio: "inherit",
			});
		} else {
	...
	public static async terminateAllAndroidEmulators(): Promise<boolean> {
		...
		if (devices.length !== 0) {
			devices.forEach((device) => {
				SmokeTestLogger.info(
					`*** Terminating Android emulator with id '${device.id}'...`
				);
				cp.execSync(`adb -s ${device.id} emu kill`, {
					stdio: "inherit",
				});
			});
		} else {
	...
	public static adbReversePort(port: number): void {
		SmokeTestLogger.info(
			`*** Forwarding port ${port} on an Android emulator.`
		);
	...
	public async waitUntilEmulatorTerminating(): Promise<boolean> {
		...
		if (result) {
			SmokeTestLogger.success(
				`*** Android emulator has been terminated.`
			);
		} else {
			SmokeTestLogger.error(
	...
	public static async waitUntilSomeEmulatorStarting(): Promise<boolean> {
		SmokeTestLogger.info("*** Wait for Android emulator starting...");
		const countOfRunningEmulators =
			AndroidEmulatorManager.getOnlineDevices().length;

		const condition = () => {
			if (
				AndroidEmulatorManager.getOnlineDevices().length >
				countOfRunningEmulators
			) {
				return true;
			}
			return false;
		...
		if (result) {
			SmokeTestLogger.success(
				`*** Some Android emulator has been started.`
			);
		} else {
			SmokeTestLogger.error(
	...
	public static async waitUntilAllEmulatorTerminating(): Promise<boolean> {
		...
		if (result) {
			SmokeTestLogger.success(
				`*** All Android emulators has been terminated.`
			);
		} else {
			SmokeTestLogger.error(
