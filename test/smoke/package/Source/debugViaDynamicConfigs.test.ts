...
export function startDebuggingViaDynamicConfigsTests(
	project: TestProject
): void {
	describe("Debugging via dynamic configs test", () => {
		...
		it("Start 'Debug Android' dynamic config", async function () {
			...
			SmokeTestLogger.info(
				"Start 'Debug Android' dynamic config test: Debugging is stopped"
			);
		});
		...
		it("Start 'Attach to packager' dynamic config", async function () {
			try {
				...
				await automationHelper.retryWithSpecifiedPollRetryParameters(
					async () => {
						return app.workbench.quickinput.selectQuickInputElement(
							0
						);
					},
					1,
