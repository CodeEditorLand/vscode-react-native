...
const requestPattern =
	/%cNetwork request:(.*?)\scolor: #0000ff\s\{(.*?)\}\s\}/gs;
...
export function startNetworkInspectorTests(
	...
	describe("Network inspector tests", () => {
		...
		function validateGetRequestResult(getRequestData: string): boolean {
			...
			const responseBodyStr =
				'  "Response Body": "GET request success: testSuccess"';
			const responseHeadersPattern =
		...
		async function networkInspectorTest(
			testname: string,
			platform: Platform
		) {
			...
			switch (platform) {
				case Platform.Android: {
					appiumOpts =
						AppiumHelper.prepareAttachOptsForAndroidActivity(
				...
				case Platform.iOS: {
					...
					await iosSimulatorManager.waitUntilIosAppIsInstalled(
						HERMES_APP_BUNDLE_ID
					);
					break;
				}
			}
			...
			if (process.env.REACT_NATIVE_TOOLS_LOGS_DIR) {
				...
				return assert.fail(
					"REACT_NATIVE_TOOLS_LOGS_DIR is not defined"
				);
			}
			...
			assert.strictEqual(
				validatePostRequestResult(requestResults[0]),
				true
			);
			assert.strictEqual(
				validateGetRequestResult(requestResults[1]),
				true
			);
			SmokeTestLogger.success(
				`${testname}: logged Network request is correct`
			);
		}
