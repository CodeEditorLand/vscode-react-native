	...
	describe("Expo tests", () => {
		...
		async function findExpoSuccessAndFailurePatterns(
			...
			const condition = () => {
				...
				SmokeTestLogger.info(
					`Searching for Expo launch logging patterns ...`
				);

				if (expoStarted || expoFailed) {
					result = { successful: expoStarted, failed: expoFailed };
					SmokeTestLogger.info(
						`Expo launch status patterns found: ${JSON.stringify(
							result,
							null,
							2
						)}`
			...
			if (result) {
				...
				SmokeTestLogger.info(
					`Expo launch logging patterns are not found`
				);
		...
		async function runExpoDebugScenario(
			...
			if (triesToLaunchApp <= 1) {
				await automationHelper.runDebugScenarioWithRetry(
					debugConfigName
				);
				await automationHelper.runCommandWithRetry(
				...
				for (let retry = 1; retry <= triesToLaunchApp; retry++) {
					...
					if (expoLaunchStatus.successful) {
						...
						if (retry === triesToLaunchApp) {
							assert.fail(
								`App start has failed after ${retry} retries`
							);
						}
						SmokeTestLogger.warn(
							`Attempt to start #${retry} failed, retrying...`
						);
		...
		async function expoTest(
			...
			await automationHelper.openFileWithRetry(
				project.projectEntryPointFile
			);
			await app.workbench.editors.scrollTop();
			SmokeTestLogger.info(
				`${testName}: ${project.projectEntryPointFile} file is opened`
			);
			await app.workbench.debug.setBreakpointOnLine(
				ExpoSetBreakpointOnLine
			);
			SmokeTestLogger.info(
			...
			await app.workbench.editors.waitForTab(
				"Expo QR Code",
				false,
				true,
				2400
			);
			await app.workbench.editors.waitForActiveTab(
				"Expo QR Code",
				false,
				true
			);
			...
			if (platform === Platform.iOSExpo) {
				...
				let appFile = findFile(
					SmokeTestsConstants.iOSExpoAppsCacheDir,
					/.*\.(app)/
				);
				if (!appFile) {
					throw new Error(
				...
				const opts = AppiumHelper.prepareAttachOptsForIosApp(
					device,
					appPath
				);
				...
				await AppiumHelper.disableDevMenuInformationalMsg(
					client,
					Platform.iOSExpo
				);
				await AppiumHelper.enableRemoteDebugJS(
					client,
					Platform.iOSExpo
				);
				await sleep(5 * 1000);
			...
			await app.workbench.editor.waitForEditorFocus(
				project.projectEntryPointFile,
				1
			);
			...
			SmokeTestLogger.success(
				`${testName}: \"Test output from debuggee\" string is found`
			);
