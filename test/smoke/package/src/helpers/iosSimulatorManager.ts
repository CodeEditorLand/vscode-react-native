...
export default class IosSimulatorManager {
	...
	public async bootSimulator(): Promise<void> {
		const cmd = "boot";
		const result = await IosSimulatorManager.runSimCtlCommand([
			cmd,
			this.simulator.name,
		]);
	...
	public async waitUntilIosSimulatorStarting(): Promise<boolean> {
		...
		const result = await waitUntil(
			condition,
			IosSimulatorManager.SIMULATOR_START_TIMEOUT
		);
		if (result) {
	...
	public async waitUntilIosAppIsInstalled(
		appBundleId: string
	): Promise<void> {
		...
		let awaitRetries: number =
			IosSimulatorManager.APP_INSTALL_AND_BUILD_TIMEOUT / 1000;
		let retry = 1;
		await new Promise<void>((resolve, reject) => {
			const check = setInterval(async () => {
				...
				if (launched) {
					clearInterval(check);
					const initTimeout =
						IosSimulatorManager.APP_INIT_TIMEOUT || 10000;
					SmokeTestLogger.success(
	...
	private findSimulator(
		...
		const foundSimulator = simulators.find(
			(value) =>
				value.name === name && (!system || value.system === system)
		);
		if (!foundSimulator) {
			return null;
		}
	...
	private static getBootedDevices(): IiOSSimulator[] {
		...
		const bootedSimulators = simulators.filter(
			(sim) => sim.state === DeviceState.Booted
		);
	...
	public static async shutdownAllSimulators(): Promise<boolean> {
		...
		if (devices.length !== 0) {
			devices.forEach((device) => {
				promises.push(
					IosSimulatorManager.shutdownSimulator(device.name)
				);
			});
		} else {
	...
	private static async shutdownSimulator(
	...
	public static async waitUntilAllIosSimulatorsTerminating(): Promise<boolean> {
		...
		if (result) {
			SmokeTestLogger.success(
				`*** All iOS simulators has been terminated.`
			);
		} else {
			SmokeTestLogger.error(
	...
	private static getTargetBuildDir(buildSettings: string) {
		const targetBuildMatch = /TARGET_BUILD_DIR = (.+)$/m.exec(
			buildSettings
		);
