...
export default class TestProject {
	...
	constructor(
		...
		private _sampleDirectory: string
	) {
		...
		this._packageJsonPath = path.join(
			this.workspaceDirectory,
			"package.json"
		);
		this._settingsJsonSamplePath = path.join(
			this.sampleDirectory,
			"settings.json"
		);
		this._settingsJsonPath = path.join(
			this._vsCodeConfigPath,
			"settings.json"
		);
		this._testButtonFileForWorkspace = path.join(
			this.workspaceDirectory,
			"AppTestButton.js"
		);
		this._testButtonFileForSample = path.join(
			this.sampleDirectory,
			"AppTestButton.js"
		);
		...
		this._metroConfigPath = path.join(
			this.workspaceDirectory,
			"metro.config.js"
		);
		this._expoSettingsPath = path.join(
			this.workspaceDirectory,
			".expo",
			"settings.json"
		);
		...
		this._gradleBuildFilePathForSample = path.join(
			this.sampleDirectory,
			"build.gradle"
		);
	}
	...
	public isExpoProject(): boolean {
		const packageJsonData = JSON.parse(
			fs.readFileSync(this.packageJsonPath).toString()
		);
		return (
			packageJsonData.dependencies.expo ||
			packageJsonData.devDependencies.expo
		);
	}
	...
	private getEntryPoint(diretory: string): string {
		let entryPoint = path.join(
			diretory,
			SmokeTestsConstants.ApptsxFileName
		);
