...
export default class AutomationHelper {
	...
	public async openFileWithRetry(
		...
		const func = async () =>
			await this.app.workbench.quickaccess.openFile(fileName);
		const catchFunc = async () =>
			await this.app.workbench.code.dispatchKeybinding("escape");
	...
	public async runCommandWithRetry(
		...
		const func = async () =>
			await this.app.workbench.quickaccess.runCommand(commandId);
		const catchFunc = async () =>
			await this.app.workbench.code.dispatchKeybinding("escape");
		await this.retryWithSpecifiedPollRetryParameters(
			func,
	...
	public async runDebugScenarioWithRetry(
		...
		const func = async () => {
			await this.app.workbench.quickaccess.runDebugScenario(
				scenario,
				index
			);
	...
	public async waitForStackFrameWithRetry(
		...
		const retryFunc = async () => {
			...
			try {
				await this.app.workbench.debug.waitForStackFrame(
					(sf: IStackFrame) => {
						stackFrame = sf;
						return func(sf);
					},
		...
		const catchFunc = async () => {
			await this.runCommandWithRetry(
				SmokeTestsConstants.reloadAppCommand
			);
		};
		await this.retryWithSpecifiedPollRetryParameters(
			retryFunc,
	...
	public async openDynamicDebugScenariosWithRetry(
		...
		const func = async () => {
			await this.app.workbench.quickaccess.openDynamicDebugScenarios(
				"React Native...",
				1
			);
