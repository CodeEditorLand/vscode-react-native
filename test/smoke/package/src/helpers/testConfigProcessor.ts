...
export class TestConfigProcessor {
	...
	private verifyEnvVariables(variables: TestEnvVariables) {
		...
		if (!variables.PURE_RN_VERSION) {
			SmokeTestLogger.warn(
				"Optional PURE_RN_VERSION variable is not set"
			);
		}
		if (!variables.PURE_EXPO_VERSION) {
			SmokeTestLogger.warn(
				"Optional PURE_EXPO_VERSION variable is not set"
			);
		}
		if (!variables.RN_MAC_OS_VERSION) {
			SmokeTestLogger.warn(
				"Optional RN_MAC_OS_VERSION variable is not set"
			);
		}
		if (!variables.RN_WINDOWS_VERSION) {
			SmokeTestLogger.warn(
				"Optional RN_WINDOWS_VERSION variable is not set"
			);
		}
		if (!variables.RNW_PACKAGE_VERSION) {
			SmokeTestLogger.warn(
				"Optional RNW_PACKAGE_VERSION variable is not set"
			);
		}
	}
