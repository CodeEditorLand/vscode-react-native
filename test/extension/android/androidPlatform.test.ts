...
suite("androidPlatform", function () {
	suite("extensionContext", function () {
		...
		const rnProjectContent = fs.readFileSync(
			ReactNative065.DEFAULT_PROJECT_FILE,
			"utf8"
		);
		...
		function createAndroidPlatform(
			runOptions: IAndroidRunOptions
		): AndroidPlatform {
		...
		setup(async () => {
			...
			adbHelper = new adb.AdbHelper(
				genericRunOptions.projectRoot,
				nodeModulesRoot
			);
			...
			getOnlineTargetsStub = sinon.stub(
				adbHelper,
				"getOnlineTargets",
				async function () {
		...
		testWithRecordings(
			...
			async () => {
				try {
					...
					should.assert(
						false,
						"runApp should've exited with an error"
					);
				} catch (error) {
					error.message
						.startsWith(
		...
		testWithRecordings(
			...
			async () => {
				...
				const isRunningOnOfflineDevice =
					devices[4].installedApplications[androidPackageName]
						.isInDebugMode === false;
		...
		testWithRecordings(
			...
			async () => {
				...
				devices = fillDevices(
					offineDevicesIds.concat(onlineDevicesIds)
				);
		...
		testWithRecordings(
			...
			async () => {
				...
				try {
					...
					should.assert(
						false,
						"Expected runApp to end up with an error"
					);
					isRunning = false;
				} catch (error) {
		...
		test("AdbHelper should correctly parse Android Sdk Location from local.properties file content", () => {
			...
			function testPaths(inputPath: string, expectedPath: string) {
				const resultPath1 = adbHelper.parseSdkLocation(
					`sdk.dir=${inputPath}`
				);
				const resultPath2 = adbHelper.parseSdkLocation(
					`sdk.dir   =${inputPath}`
				);
				const resultPath3 = adbHelper.parseSdkLocation(
					`sdk.dir = ${inputPath}`
				);
			...
			testPaths(
				String.raw`\\\\Network\\Shared\\Folder`,
				String.raw`\\Network\Shared\Folder`
			);
			testPaths(
			...
			mockPlatform("darwin");
			testPaths(
				String.raw`/var/lib/some/path`,
				String.raw`/var/lib/some/path`
			);
			testPaths(String.raw`~/Library`, String.raw`~/Library`);
			testPaths(
				String.raw`/Users/User1/home/path`,
				String.raw`/Users/User1/home/path`
			);
			testPaths(
		...
		test("AdbHelper parseSdkLocation function should correctly parse Android Sdk Location from local.properties and wrap with quotes", () => {
			function testPaths(expectedPath: string, projectRoot: string) {
				...
				const fileContent = fs
					.readFileSync(localPropertiesFilePath)
					.toString();
