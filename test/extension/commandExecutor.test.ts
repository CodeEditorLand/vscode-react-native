...
suite("commandExecutor", function () {
	suite("extensionContext", function () {
		...
		teardown(function () {
			const mockedMethods = [
				Log.log,
				...Object.keys(childProcessStubInstance),
			];
		...
		setup(() => {
			childProcessStub = sinon
				...
				.returns(childProcessStubInstance) as ChildProcess &
				Sinon.SinonStub;
		...
		test("should execute a command", async function () {
			...
			sinon.stub(
				Log,
				"log",
				function (message: string, formatMessage: boolean = true) {
		...
		test("should reject on good command that fails", async () => {
			...
			try {
				...
				assert.fail(
					null,
					null,
					"node should not be able to install bad-package"
				);
			} catch (reason) {
				console.log(reason.message);
				assert.strictEqual(reason.errorCode, 101);
		...
		test("should return correct CLI react command", async () => {
			...
			const command = HostPlatform.getNpmCliCommand(
				ce.selectReactNativeCLI()
			);
		...
		test("should not fail on react-native command without arguments", async () => {
			...
			try {
				await new CommandExecutor(nodeModulesRoot).spawnReactCommand(
					"run-ios"
				).outcome;
			} catch (error) {
		...
		suite("ReactNativeClIApproaches", function () {
			...
			test("selectReactNativeCLI should return local CLI", (done: Mocha.Done) => {
				...
				assert.strictEqual(
					commandExecutor.selectReactNativeCLI(),
					localCLIPath
				);
				done();
			});
			...
			test("selectReactNativeCLI should return global CLI", (done: Mocha.Done) => {
				const randomHash = new Crypto().hash(
