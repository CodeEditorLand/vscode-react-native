...
suite("experimentService", function () {
	...
	suite("initializationAndExperimentConfig", function () {
		test("should return correct experiment config", async () => {
			...
			let downloadedExperimentsConfig: ExperimentConfig[] = await (<any>(
				experimentService
	...
	suite("executeExperiment", function () {
		...
		test("should skip the experiment", async () => {
			...
			let experimentResult: ExperimentResult =
				await experimentService.executeExperiment(expTestConfig);
			assert.strictEqual(
				experimentResult.resultStatus,
				ExperimentStatuses.DISABLED
			);
		});

		test("should succeed the experiment", async () => {
			let experimentService = <any>ExperimentService.create();
			await configureExperimentService(
				experimentService,
				RNTPreviewPromptExp
			);
			let experimentResult: ExperimentResult =
				await experimentService.executeExperiment(RNTPreviewPromptExp);
			assert.strictEqual(
				experimentResult.resultStatus,
				ExperimentStatuses.ENABLED
			);
		});
	});
});
