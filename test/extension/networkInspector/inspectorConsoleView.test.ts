...
import {
	Response,
	Request,
} from "../../../src/extension/networkInspector/networkMessageData";
import { URL } from "url";
...
suite("inspectorConsoleView", function () {
	suite("createNetworkRequestData", function () {
		...
		test("should return network request data with text response body", () => {
			...
			const networkRequestData = (<any>(
				inspectorConsoleView
			)).createNetworkRequestData(request, response);

			assert.deepStrictEqual(
				networkRequestData,
				networkRequestDataReference
			);
		});
		...
		test("should correctly process a graphQL request", () => {
			...
			const networkRequestDataReference = {
				...
				networkRequestData: {
					...
					"Response Body": {
						data: { rates: [{ currency: "AED", rate: "3.673" }] },
					},
				},
			};
		...
		test("should correctly process a request with form data", () => {
			...
			const networkRequestDataReference = {
				...
				networkRequestData: {
					...
					"Request Body": querystring.parse(
						"userName=testName&testProp=testPropVal"
					),
					"Response Body": { success: true },
				},
			};
