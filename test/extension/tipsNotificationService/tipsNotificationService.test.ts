...
suite("tipNotificationService", function () {
	...
	suite("initializeTipsConfig", function () {
		test("should create correct tips config", async () => {
			...
			const tipsConfig: TipsConfig = (<any>tipNotificationService)
				.tipsConfig;

			const numberType = "number";
			assert.strictEqual(
				typeof tipsConfig.daysAfterLastUsage,
				numberType
			);
			assert.strictEqual(
				typeof tipsConfig.firstTimeMaxDaysToRemind,
				numberType
			);
			assert.strictEqual(
				typeof tipsConfig.firstTimeMinDaysToRemind,
				numberType
			);
	...
	suite("showTipNotification", function () {
		suite("without user actions", function () {
			...
			test("should create config and fill shownDate into one of specific tips", async () => {
				...
				assert.strictEqual(
					typeof tipsConfig.tips.specificTips[specificTipKey]
						.shownDate,
					"string",
			...
			test("should decrease daysLeftBeforeGeneralTip by one and no tip is shown", async () => {
				await (<any>tipNotificationService).initializeTipsConfig();
				const tipsConfigBefore: TipsConfig = (<any>(
					tipNotificationService
				)).tipsConfig;
				...
				const tipsConfigAfter: RawTipsConfig =
					config.get(tipsConfigName);

				assert.deepStrictEqual(
					tipsConfigBefore.tips,
					tipsConfigAfter.tips
				);
				assert.deepStrictEqual(
			...
			test("should show another tip from general tips for the first round", async () => {
				...
				const tipsConfigAfter: RawTipsConfig =
					config.get(tipsConfigName);
				let shownTips = Object.values(
					tipsConfigAfter.tips.generalTips
				).filter((tip) => tip.shownDate);
			...
			test("should show another tip from general tips for the second round", async () => {
				...
				Object.values(tipsConfigBefore.tips.generalTips).forEach(
					(tip) => {
				...
				let shownTips = Object.values(
					...
					(tip) =>
						Date.parse(tip.shownDate as string) >
						shownDate.getTime()
				);
				...
				let assertCondition = () =>
					tipsConfigAfter.daysLeftBeforeGeneralTip >=
						tipsConfigAfter.minDaysToRemind &&
					tipsConfigAfter.daysLeftBeforeGeneralTip <=
						tipsConfigAfter.maxDaysToRemind;
			...
			test("should change allTipsShownFirstly from false to true in case all general tips have been shown for the first time", async () => {
				...
				const shownDate = new Date(2021, 6, 26);
				const generalTipsKeysBefore = Object.keys(
					tipsConfigBefore.tips.generalTips
				);
				...
				const shownTips = Object.values(
					tipsConfigAfter.tips.generalTips
				).filter((tip) => tip.shownDate);

				assert.strictEqual(
					shownTips.length,
					generalTipsKeysBefore.length
				);
			...
			test("should not show a general tip", async () => {
				...
				const shownGeneralTips = Object.values(
					tipsConfigAfter.tips.generalTips
				).filter((tip) => tip.shownDate);
				const shownSpecificTips = Object.values(
					tipsConfigAfter.tips.specificTips
				).filter((tip) => tip.shownDate);
		...
		suite("with user actions", function () {
			test("should not show tips after the user has disabled the display of tips", async () => {
				...
				const tipsConfigAfterDisplayingTip: RawTipsConfig =
					config.get(tipsConfigName);
				...
				assert.strictEqual(
					shownGeneralTipsAfterDisplayingTip.length,
					1
				);
	...
	suite("setKnownDateForFeatureById", function () {
		test("should add knownDate to a general tip", async () => {
			...
			const tipsConfigAfterAddingKnownDate: RawTipsConfig =
				config.get(tipsConfigName);

			assert.strictEqual(
				typeof tipsConfigAfterAddingKnownDate.tips.generalTips[tipKey]
					.knownDate,
				"string",
			...
			const tipsConfigAfterUpdatingKnownDate: RawTipsConfig =
				config.get(tipsConfigName);

			const addedKnownDate = new Date(
				tipsConfigAfterAddingKnownDate.tips.generalTips[tipKey]
					.knownDate as string
			);
			const updatedKnownDate = new Date(
				tipsConfigAfterUpdatingKnownDate.tips.generalTips[tipKey]
					.knownDate as string
			);

			assert.strictEqual(
				updatedKnownDate.getTime() > addedKnownDate.getTime(),
				true
			);
		});
		...
		test("should add knownDate to a specific tip", async () => {
			...
			await tipNotificationService.setKnownDateForFeatureById(
				tipKey,
				false
			);
	...
	suite("updateTipsConfig", function () {
		...
		test("should update config after deleting a tip from storage", async () => {
			...
			const mockedTipsNotificationServiceAfter = proxyquire(
				tipsNotificationServicePath,
				{
					"./tipsStorage": {
