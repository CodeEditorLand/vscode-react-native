...
suite("plistBuddy", function () {
	suite("extensionContext", function () {
		...
		test("setPlistProperty should attempt to modify, then add, plist properties", async function () {
			...
			mockedExecFunc.withArgs(setCallArgs).returns(
				Promise.resolve({
					outcome: Promise.reject(
						new Error("Setting does not exist")
					),
				})
			);
			mockedExecFunc
				...
				.returns(
					Promise.resolve({ outcome: Promise.resolve("stdout") })
				);
			...
			await plistBuddy.setPlistProperty(
				plistFileName,
				plistProperty,
				plistValue
			);
		...
		test("getBundleId should return the bundle ID for RN <0.59", async function () {
			...
			const [simulatorId1, simulatorId2, deviceId1, deviceId2] =
				await Promise.all([
		...
		suite("fetchParameterFromBuildSettings", function () {
			...
			test("fetchParameterFromBuildSettings should return parameter value", function () {
				...
				const targetBuildDir =
					plistBuddy.fetchParameterFromBuildSettings(
		...
		function getPlistBuddy(
			...
			mockedExecFunc
				...
				.returns(
					Promise.resolve({
						outcome: Promise.resolve(simulatorBundleId),
					})
				);
			mockedExecFunc
				...
				.returns(
					Promise.resolve({
						outcome: Promise.resolve(deviceBundleId),
					})
				);
			const mockChildProcess: any = {
				exec: mockedExecFunc,
