...
import {
	InternalError,
	NestedError,
} from "../../src/common/error/internalError";
...
suite("MobileTargetManager", function () {
	...
	const getPathWithoutEmulator = PromiseUtil.promiseCacheDecorator(
		async () => {
			const cp = new ChildProcess();
			const isWin = process.platform === "win32";
			try {
				const whereEmulatorOutput = isWin
					? await cp.execToString("where emulator", {
							env: process.env,
					  })
					: await cp.execToString("which -a emulator", {
							env: process.env,
					  });
				...
				return defaultPath
					...
					.filter(
						(path) =>
							!pathsToEmulatorUtility.find(
								(emuPath) => emuPath === path
							)
					)
					.join(isWin ? ";" : ":");
			} catch (error) {
		...
		test("Should properly recognize virtual target type", async function () {
			...
			await checkTargetType(
				async () =>
					assert.strictEqual(
						await targetManager.isVirtualTarget(
							onlineSimulator2.name as string
						),
						true,
						`Could not recognize simulator name: ${
							onlineSimulator2.name as string
						}`
					),
				() =>
	...
	function runTargetSelectionTests() {
		...
		test("Should show targets by filter", async function () {
			const onlineTargetsFilter = (target: IMobileTarget) =>
				target.isOnline;
			await checkTargetSeletionResult(
				onlineTargetsFilter,
		...
		suiteSetup(() => {
			...
			launchSimulatorStub = Sinon.stub(
				...
				async (simulator: IMobileTarget) => {
					...
					return IOSTarget.fromInterface(
						<IDebuggableIOSTarget>simulator
					);
				}
			);
			targetsForSelection = [];
			...
			launchSimulatorStub = Sinon.stub(
				...
				async (simulatorTarget: IMobileTarget) => {
					...
					return AndroidTarget.fromInterface(
						<IDebuggableMobileTarget>simulatorTarget
					);
				}
			);
			getAvdsNamesStub = Sinon.stub(
				adbHelper,
				"getAvdsNames",
				async () => {
					return [
				...
				test(`Should not throw an error for the target equals ${TargetType.Simulator}`, async function () {
					await executeWithoutEmulator(async () => {
						await checkTargetType(
							...
							(err) =>
								assert.fail(`Error has been thrown: ${err}`)
						);
					});
				});
					...
					await executeWithoutEmulator(async () => {
						await checkTargetType(
							async () => {
								...
								assert.fail(
									`Did not throw error and return ${isVirtualTarget}`
								);
							},
							(err: Error) => {
