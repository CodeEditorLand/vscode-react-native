...
suite("sourceMap", function () {
	...
	test("should ignore sourcemap urls in JavaScript objects and strings", function () {
		const expectedUrlHref =
			"http://localhost:8081/index.map?platform=android&dev=true";
		const sourceMap = new SourceMapUtil();
	...
	test("should resolve a valid sourcemap url", function () {
		...
		const expectedUrlHref =
			"http://localhost:8081/index.ios.map?platform=ios&dev=true";
	...
	test("should return correct sourcemap url for RN macOS", function () {
		...
		const expectedUrlHref =
			"http://localhost:8081/index.map?platform=macos&dev=true";
	...
	test("should update scripts with source mapping urls", function () {
		const scriptBody: string =
			"//# sourceMappingURL=/index.ios.map?platform=ios&dev=true";
		const sourceMappingUrl: IStrictUrl = <IStrictUrl>(
			url.parse("/index.android.map")
		);
		const expectedScriptBody = "//# sourceMappingURL=index.android.map";
		const sourceMap = new SourceMapUtil();

		const result = sourceMap.updateScriptPaths(
			scriptBody,
			sourceMappingUrl
		);
	...
	test("should update absolute source path to relative unix style path", function () {
		...
		const result = (<any>sourceMap).updateSourceMapPath(
			sourcePath,
			sourcesRootPath
		);
