...
suite("sourceMapsCombinator", function () {
	suite("#convert", function () {
		...
		const codeJS = fs.readFileSync(
			path.resolve(__dirname, "assets/hello.js")
		);
		const codeTS = fs.readFileSync(
			path.resolve(__dirname, "assets/hello.ts")
		);
		...
		setup(() => {
			...
			fsReadFileStub
				.withArgs(sourcemapPath)
				.returns(JSON.stringify(sourcemap));
		});
