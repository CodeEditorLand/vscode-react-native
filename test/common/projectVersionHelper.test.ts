...
} from "../../src/common/projectVersionHelper";
...
suite("projectVersionHelper", function () {
	...
	test("getReactNativeVersionsFromProjectPackage should return object containing version strings if 'version' field is found in project's package.json file", async () => {
		...
		const versions =
			await ProjectVersionHelper.getReactNativeVersionsFromProjectPackage(
	...
	test("isVersionError should return true if a version string contains an error substring", (done: Mocha.Done) => {
		...
		assert.strictEqual(
			ProjectVersionHelper.isVersionError(
				RN_VERSION_ERRORS.UNKNOWN_ERROR
			),
			true
		);
		assert.strictEqual(
			ProjectVersionHelper.isVersionError("someError"),
			true
		);
		assert.strictEqual(
			ProjectVersionHelper.isVersionError("ERRORSTRING"),
			true
		);
	...
	test("isVersionError should return false if a version string doesn't contain an error substring", (done: Mocha.Done) => {
		assert.strictEqual(
			ProjectVersionHelper.isVersionError("^0.60.0-vnext.68"),
			false
		);
		assert.strictEqual(
			ProjectVersionHelper.isVersionError(
		...
		assert.strictEqual(
			ProjectVersionHelper.isVersionError("SemverInvalid"),
			false
		);
		assert.strictEqual(
			ProjectVersionHelper.isVersionError("0.61.0"),
			false
		);
		assert.strictEqual(
			ProjectVersionHelper.isVersionError("SemverInvalid: URL"),
			false
		);
	...
	test("processVersion should return semver valid version strings or correct error strings", (done: Mocha.Done) => {
		...
		assert.strictEqual(
			ProjectVersionHelper.processVersion("latest", false),
			"SemverInvalid"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("~1.2", false),
			"SemverInvalid"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("2.x", false),
			"SemverInvalid"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion(">=1.0.2", false),
			"1.0.2"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("<3.0.0", false),
			"3.0.0"
		);

		assert.strictEqual(
			ProjectVersionHelper.processVersion("0.61.0-rc.0"),
			"0.61.0"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("~1.2.3-beta.1"),
			"1.2.3"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("~0.61.3-dev"),
			"0.61.3"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("v0.61.3-dev"),
			"0.61.3"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("^0.61.2.0"),
			"0.61.2"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("0.61.3"),
			"0.61.3"
		);
		...
		assert.strictEqual(
			ProjectVersionHelper.processVersion(
				"git+https://git@github.com/test/test.git"
			),
			"SemverInvalid: URL"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion(
				"/github.com/expo/react-native/archive/sdk"
			),
			"SemverInvalid"
		);
		assert.strictEqual(
		...
		assert.strictEqual(
			ProjectVersionHelper.processVersion("^str.0.61.str.2"),
			"0.61.0"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("qwert  1 asdf"),
			"1.0.0"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("^0.str.str.2"),
			"0.0.0"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion(""),
		...
		assert.strictEqual(
			ProjectVersionHelper.processVersion("1.0.x"),
			"1.0.0"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("*.0.*"),
			"0.0.0"
		);
		assert.strictEqual(
			ProjectVersionHelper.processVersion("~1.*"),
			"1.0.0"
		);
