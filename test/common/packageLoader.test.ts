...
suite("packageLoader", async () => {
	...
	function isNotEmptyPackage(pck: any): boolean {
		return (
			pck && (Object.keys(pck).length !== 0 || typeof pck === "function")
		);
	}
	...
	suite("localNodeModules", async () => {
		...
		const sampleProjectNodeModulesPath = path.join(
			sampleProjectPath,
			"node_modules"
		);
		const sampleProjectPackageLockJsonPath = path.join(
			sampleProjectPath,
			"package-lock.json"
		);
		...
		const chalkPackageConfig = new PackageConfig(
			"chalk",
			"4.1.1",
			"./source/util.js"
		);
		...
		suiteSetup(async function () {
			...
			getVersionFromExtensionNodeModulesStub = sinon.stub(
				...
				(packageName: string) => {
					return getPackageVersionsFromNodeModules(
						sampleProjectPath,
						packageName
					);
				}
		...
		test("The package loader should install packages in node_modules where these packages are not present", async function () {
			...
			const getMkdrip =
				PackageLoader.getInstance().generateGetPackageFunction<any>(
					mkdirpPackageConfig,
					rimrafPackageFirst
				);
			const getRimraf =
			...
			const [mkdirpPackage, rimrafPackage] = await Promise.all([
				getMkdrip(),
				getRimraf(),
			]);

			assert.strictEqual(
				isNotEmptyPackage(rimrafPackage) &&
					isNotEmptyPackage(mkdirpPackage),
				true,
				"Not all packages has been installed and required"
			);

			const installedVersionOfMkdirp =
				await getPackageVersionsFromNodeModules(
					sampleProjectPath,
					mkdirpPackageConfig.getPackageName()
				);
			const installedVersionOfRimraf =
				await getPackageVersionsFromNodeModules(
					sampleProjectPath,
		...
		test("The package loader should install package and require specific subpath for this package", async function () {
			...
			assert.strictEqual(
				isNotEmptyPackage(chalkPackage),
				true,
				"Package was not required"
			);
			assert.strictEqual(
				!!(
					chalkPackage.stringReplaceAll &&
					chalkPackage.stringEncaseCRLFWithFirstIndex
				),
				true,
