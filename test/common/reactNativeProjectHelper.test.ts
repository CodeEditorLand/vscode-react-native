...
suite("ReactNativeProjectHelper", function () {
	...
	suite("isAndroidHermesEnabled", () => {
		...
		suiteSetup(() => {
			fsHelper.makeDirectoryRecursiveSync(
				path.dirname(buildGradleFilePath)
			);
		});

		suiteTeardown(() => {
			fsHelper.removePathRecursivelySync(
				path.join(sampleReactNativeProjectDir, "android")
			);
		});
		...
		test("isAndroidHermesEnabled should return 'true' if Hermes engine is enabled in the build.gradle file", () => {
			...
			const androidHermesEnabled =
				ReactNativeProjectHelper.isAndroidHermesEnabled(
					sampleReactNativeProjectDir
				);
	...
	suite("isIOSHermesEnabled", () => {
		const podfileFilePath = path.join(
			sampleReactNativeProjectDir,
			"ios",
			"Podfile"
		);
		...
		suiteTeardown(() => {
			fsHelper.removePathRecursivelySync(
				path.join(sampleReactNativeProjectDir, "ios")
			);
		});
		...
		test("isIOSHermesEnabled should return 'true' if Hermes engine is enabled in the Podfile file", () => {
			...
			const iOSHermesEnabled =
				ReactNativeProjectHelper.isIOSHermesEnabled(
					sampleReactNativeProjectDir
				);
	...
	suite("isMacOSHermesEnabled", () => {
		...
		suiteTeardown(() => {
			fsHelper.removePathRecursivelySync(
				path.join(sampleReactNativeProjectDir, "macos")
			);
		});
		...
		test("isMacOSHermesEnabled should return 'true' if the 'hermes_enabled' parameter in the Podfile file is uncommented and equal to 'true'", () => {
			...
			const macOSHermesEnabled =
				ReactNativeProjectHelper.isMacOSHermesEnabled(
					sampleReactNativeProjectDir
				);
	...
	suite("isWindowsHermesEnabled", () => {
		...
		suiteSetup(() => {
			fsHelper.makeDirectoryRecursiveSync(
				path.dirname(experimentalFeaturesFilePath)
			);
		});

		suiteTeardown(() => {
			fsHelper.removePathRecursivelySync(
				path.join(sampleReactNativeProjectDir, "windows")
			);
		});
		...
		test("isWindowsHermesEnabled should return 'true' if Hermes engine is enabled in the ExperimentalFeatures.props file", () => {
			const experimentalFeaturesFileContent =
				...
				"  </PropertyGroup>";
			fs.writeFileSync(
				experimentalFeaturesFilePath,
				experimentalFeaturesFileContent
			);

			const windowsHermesEnabled =
				ReactNativeProjectHelper.isWindowsHermesEnabled(
					sampleReactNativeProjectDir
				);
